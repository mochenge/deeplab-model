...
_PASCAL = 'pascal'
_MYDATA = 'mydata' # 添加在这里，和注册的数据集名字相同
...
#---------------------------------------------------#
#     接下来为自己数据集写一个函数，用途是返回一个np.
#     ndarray对象，尺寸为[classes, 3] ，即colormap
#     共有classes种RGB颜色，分别代表不同的类别。
#---------------------------------------------------#
...
#---------------------------------------------------#
#    自己的数据集共定义了6中colormap，即：colormap为[6,3]
#---------------------------------------------------#
...
def create_label_colormap(dataset=_PASCAL):
  ...
  if dataset == _ADE20K:
    ...
    return create_mydata_label_colormap()  # 添加在这里
  else:
