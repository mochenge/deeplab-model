...
sys.path.append(r'E:\PycharmProject\DeepLab_model\models\research\slim')  # 以上两处的路径请按照真实的路径添加
...
# num_clones: GPU显卡使用数量，可设置为1,2，4，若为3会报错
...
#---------------------------------------------------#
#   训练模型输出文件位置:
#   如：train_logdir = RoadScene/exp/train_on_train_set/train
#---------------------------------------------------#
...
#---------------------------------------------------#
#   train_batch_size参数设置
#   建议train_batch_size数量为显卡数量的倍数较好
#---------------------------------------------------#
...
#---------------------------------------------------#
#   加载的与训练模型权重
#   如：deeplabv3_cityscapes_train/model.ckpt
#---------------------------------------------------#
...
#---------------------------------------------------#
#   fine_tune_batch_norm参数设置
#   模型结果复现，需要使用大 BatchSize (>12), 并设置
#   fine_tune_batch_norm = True.
#   如果GPU内存有限，则BatchSize设置小一点，并设置
...
#   output_stride = 32 --> atrous_rates = [3, 6, 9]
#   mobilenet_v2：
#   output_stride = 32  --> atrous_rates = None
#   output_stride = 8  --> atrous_rates = None
#   output_stride = 16 --> atrous_rates = None
#---------------------------------------------------#
...
# 在训练过程中，用于计算损失值的前K个百分比像素点。该指标对硬像素点挖掘很有用
flags.DEFINE_float(
    'top_k_percent_pixels', 1.0,
    'The top k percent pixels (in terms of the loss values根据损失值) used to compute '
...
#---------------------------------------------------#
#   dataset参数设置
#   数据集需要在deeplab/datasets/data_generator.py文件中注册：
...
#---------------------------------------------------#
#   train_split参数配置：
#   如：train_split="train", or "val", or "test"
#---------------------------------------------------#
...
#---------------------------------------------------#
#   TFRecord格式数据集
#   如：dataset_dir='RoadScene\tfrecord'
#---------------------------------------------------#
...
if __name__ == '__main__':
  ...
  # train_logdir训练过程中的模型参数保存位置
  flags.mark_flag_as_required('train_logdir')
  # dataset_dir为TFRecord格式训练数据集
